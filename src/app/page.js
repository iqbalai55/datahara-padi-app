...
import { getFieldFromDocument } from './utils/firestore';
import { getPlants } from './utils/baseline';

export default function Home() {
  const [hoveredCard, setHoveredCard] = useState(null);
  const [plants, setPlants] = useState([]);
  const [hstMap, setHstMap] = useState({});
  ...
  useEffect(() => {
    const fetchHst = async () => {
      ...
      for (const plant of plants) {
        try {
          const { field } = await getFieldFromDocument("status_tanaman", plant.id, "hst");
          if (field !== null && field !== undefined) newMap[plant.id] = field;
        } catch {}
      }
      setHstMap(newMap);
    };
    if (plants.length > 0) fetchHst();
  }, [plants]);
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-gray-600 capitalize mt-1">
                      {hstMap[plant.id] !== undefined ? `${hstMap[plant.id]} HST` : 'Loading...'}
                    </p>
                  </div>
